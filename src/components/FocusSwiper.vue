<template>
  <div>
    <swiper :options="swiperOption" ref="mySwiper">
      <!-- slides -->
      <swiper-slide :key="item.index" v-for="item in indexFocus">
        <IndexFocus  :focus="item.name" :icon="item.icon" :name="item.name" :where="item.name">
        </IndexFocus>
      </swiper-slide>
      <!-- Optional controls -->
...
<script>
import IndexFocus from '@/components/IndexFocus'
